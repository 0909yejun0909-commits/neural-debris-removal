...
# --- step17b_emb_235.py (CPU - RELIABLE - 235 BASE - FIXED NP SCOPE) ---
STEP17B_CODE = r'''
import os
import json
import gc
...
log("V17B START (CPU MODE, 235 BASE)")
...
def find_paths():
    ...
    # MODIFIED: Use the 235.62 base CSV
    csv_candidates = list(Path("/kaggle/input").rglob("simple-ft_rescue_lf0.2_dm0.05.csv"))
...
def get_level_for_bbox(bbox):
    ...
    lvl = int(np.floor(4 + np.log2(s / 224 + 1e-6)))
    return np.clip(lvl, 3, 7)
...
def run():
    ...
    for img_id, anns in img_to_anns.items():
        ...
        for ann in anns:
            x, y, w, h = ann["bbox"]
            lvl = get_level_for_bbox([x, y, x+w, y+h])
    ...
    log(f"Extracted {len(templates)} templates")
    ...
    # ADDED: Distribution diagnostics
    ...
    for name, arr in [("All", scored_df["sim"].values), ("Uncond (>=0.6)", uncond), ("Rescued (<0.6)", rescued)]:
        ...
        log(f"{name:15s} n={len(arr):4d} min={arr.min():.3f} p10={p[0]:.3f} p25={p[1]:.3f} med={p[2]:.3f} p75={p[3]:.3f} p90={p[4]:.3f} p95={p[5]:.3f} max={arr.max():.3f}")

    # ADDED: Data-driven thresholds
    THRESHOLDS = [np.percentile(scored_df["sim"], q) for q in [50, 60, 65, 70, 75, 80, 85, 90, 95]]
    ...
    for T in THRESHOLDS:
        ...
        for _, row in df.iterrows():
            ...
            # Reconstruct prediction string from scored rows for this image
        ...
        out_name = f"filter_emb_T{T:.4f}.csv"
...
def main():
    Path("step17_embedding_dist.py").write_text(STEP17B_CODE)
    import subprocess
    subprocess.run(["python", "step17_embedding_dist.py"])
